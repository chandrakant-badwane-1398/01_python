...
def export_table_to_s3():
    ...
    try:
        ...
        # Query only updated or inserted rows since ETL batch date
        query = f"""
            SELECT {columns}
        ...
        # Upload CSV to S3
        s3 = boto3.client("s3")
        s3_key = f"{table_name}/{schema}/order.csv"
        s3.put_object(Bucket=bucket_name, Key=s3_key, Body=csv_buffer.getvalue())

        print(f"order.csv uploaded successfully to s3://{bucket_name}/{s3_key}")
