...
def load_customers_to_redshift():
    load_dotenv()
    ...
    # get batch date once from metadata schema
    ...
    s3_key = f"CUSTOMERS/{etl_batch_date}/customers.csv"
    s3_path = f"s3://{bucket}/{s3_key}"

    print("Loading from:", s3_path)

    copy_sql = f"""
        COPY {stage_schema}.customers
        FROM '{s3_path}'
        IAM_ROLE '{role}'
        IGNOREHEADER 1
        BLANKSASNULL
