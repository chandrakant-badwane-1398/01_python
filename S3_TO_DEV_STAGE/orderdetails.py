...
def load_orderdetails_to_redshift():
    load_dotenv()
    ...
    s3_key  = f"ORDERDETAILS/{etl_batch_date}/orderdetails.csv"
    s3_path = f"s3://{bucket}/{s3_key}"

    print("Loading from:", s3_path)

    copy_sql = f"""
        COPY {stage_schema}.orderdetails
        FROM '{s3_path}'
        IAM_ROLE '{role}'
        IGNOREHEADER 1
        BLANKSASNULL
    ...
    with psycopg2.connect(host=host, port=port, dbname=database, user=user, password=password) as conn:
        with conn.cursor() as cur:
            try:
                ...
                conn.commit()
                print("ORDERDETAILS LOAD DONE")
            except Exception as e:
                print("COPY ERROR:", e)


if __name__ == "__main__":
    load_orderdetails_to_redshift()
